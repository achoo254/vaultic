// Register page — email + password + confirm -> create account
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing.lg }}>
        ...
        <div>
          <div style={{ position: 'relative' }}>
            <Input label="Master Password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Choose a strong password" required />
            <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: 12, top: 30, background: 'none', border: 'none', cursor: 'pointer', padding: 4 }} tabIndex={-1}>
              {showPassword ? <IconEyeOff size={16} stroke={1.5} /> : <IconEye size={16} stroke={1.5} />}
            </button>
          </div>
          {password && (
            <div style={{ height: 4, borderRadius: 2, background: colors.border, marginTop: 6, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${strength.percent}%`, background: strength.color, borderRadius: 2, transition: 'width 0.2s' }} />
            </div>
          )}
        </div>
...
function getPasswordStrength(pw: string, colors: { error: string; warning: string; success: string; border: string }) {
  if (!pw.length) return { percent: 0, color: colors.border };
  let score = 0;
